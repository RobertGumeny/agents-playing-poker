...
- If an `action` reply does not arrive before the deadline, the server forces the safest legal action available, preferring `check` when legal and otherwise using `fold`. It persists that as `auto_check` or `auto_fold` in `hands.jsonl` with `forced_reason: "decision_timeout"`, and continues.
