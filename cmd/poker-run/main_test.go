...
func TestAgentAliasResolverResolvesLLMAkgDurableWithAbsolutePathsAndEnv(t *testing.T) {
	t.Parallel()
	...
	resolver := agentAliasResolver{
		...
		lookPath: func(string) (string, error) { return "/usr/bin/node", nil },
		buildBinary: func(repoDir, goBinary, pkg, outputPath string) error {
			t.Fatalf("buildBinary called unexpectedly for llm-akg-durable")
			return nil
		},
	}

	spec, err := resolver.resolve(llmAkgDurableAlias, "anthropic:claude-sonnet-4", "high")
	if err != nil {
		t.Fatalf("resolve() error = %v", err)
	}
	if spec.Name != llmAkgDurableAlias {
		t.Fatalf("spec.Name = %q, want %q", spec.Name, llmAkgDurableAlias)
	}
	if spec.Command != "/usr/bin/node" {
		t.Fatalf("spec.Command = %q, want /usr/bin/node", spec.Command)
	}
	...
	if !strings.HasSuffix(spec.Args[0], filepath.Join("pi-agents", "llm-akg-durable", "dist", "main.js")) {
		t.Fatalf("spec.Args[0] = %q, want llm-akg-durable dist entrypoint", spec.Args[0])
	}
	wantEnv := []string{
		"PI_POKER_FAKE_DECISIONS_JSON=",
		"PI_POKER_THINKING_LEVEL=high",
		"PI_POKER_MODEL=anthropic:claude-sonnet-4",
	}
	if strings.Join(spec.Env, "\n") != strings.Join(wantEnv, "\n") {
		t.Fatalf("spec.Env = %q, want %q", spec.Env, wantEnv)
