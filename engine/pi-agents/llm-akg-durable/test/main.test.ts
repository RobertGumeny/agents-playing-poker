...
import { ROOT_ID, ROOT_TYPE, STORE_FILE } from "../src/graph.js";
...
describe("llm-akg-durable package wiring", () => {
  it("publishes a stable poker-server entrypoint", async () => {
    ...
    expect(pkg.bin).toEqual({ "poker-agent-llm-akg-durable": "./dist/main.js" });
  });

  it("runs as a subprocess, updates the graph after a hand, and injects the root node into the next decision", async () => {
    const { spawn } = await import("node:child_process");
    const sessionDir = await mkdtemp(path.join(tmpdir(), "llm-akg-durable-session-"));
    ...
    const messages: Envelope[] = [
      ...
      },
      {
        ...
        payload: {
          ...
          legal_actions: [{ action: "fold" }, { action: "call", amount: 2 }],
        },
      },
      {
        v: 1,
      ...
      { v: 1, type: "session_end", id: "msg-7", payload: {} },
    ];
    ...
    expect(decoded[0]).toMatchObject({ type: "session_ready", in_reply_to: "msg-1", payload: { version: "llm-akg-durable/0.1.0" } });
    ...
    // The post-hand update wrote a hand node and linked it from the root.
    const store = await open(path.join(sessionDir, STORE_FILE));
    const hand = store.getNode("hand", "hand-1");
    expect(hand).not.toBeNull();
    expect(store.getNode(ROOT_TYPE, ROOT_ID)!.body).toContain("hand=1 | hero_pos=sb/button | hero_hole=As Kh");
    expect(store.outboundEdges({ type: ROOT_TYPE, id: ROOT_ID }, "has_hand")).toHaveLength(1);
    ...
    // The second decision prompt injects the updated root node body.
