...
export async function openStore(memoryDir: string): Promise<Store> {
...
export function ensureRootNode(store: Store): void {
  if (store.getNode(ROOT_TYPE, ROOT_ID)) return;
  store.putNode(ROOT_TYPE, ROOT_ID, { title: ROOT_ID, body: ROOT_SEED_BODY }, [ROOT_TYPE]);
}

export function readRootBody(store: Store): string {
  return store.getNode(ROOT_TYPE, ROOT_ID)?.body ?? "";
}

export function isRoot(node: { type: string; id: string }): boolean {
...
export function countGraphRot(store: Store): GraphRot {
