...
export function createReadTools(getStore: StoreProvider): ToolDefinition[] {
  return [
    ...
    defineTool({
      ...
      label: "AKG Node",
      description:
        "Return a node's title, body, meta, tags, plus its outbound and inbound edges. Follow an edge by reading its other endpoint. Returns found:false for a missing node.",
      parameters: Type.Object({
      ...
      async execute(_toolCallId, params) {
        ...
        if (!store || !node) return jsonResult({ found: false, type: params.type, id: params.id });
...
export function createWriteTools(getStore: StoreProvider): ToolDefinition[] {
  return [
    defineTool({
      ...
      label: "AKG Put Node",
      description:
        "Create or replace a node. You choose the type, id, title, body, meta, and tags. Reuse an existing (type, id) to update it in place. Does not delete nodes.",
      parameters: Type.Object({
    ...
    defineTool({
      ...
      label: "AKG Put Edge",
      description:
        "Create or replace a directed edge between two existing nodes. You choose the relation. Both endpoint nodes must already exist (create them with akg_put_node first). Does not delete edges.",
      parameters: Type.Object({
