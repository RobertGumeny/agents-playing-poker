...
export function createReadTools(getStore) {
    return [
        ...
        defineTool({
            name: "akg_get_node",
            label: "AKG Node",
            description: "Return a node's title, body, meta, tags, plus its outbound and inbound edges. Follow an edge by reading its other endpoint. Returns found:false for a missing node. To read several nodes at once, prefer akg_get_nodes.",
            parameters: Type.Object({
        ...
        defineTool({
            ...
            description: "Read several nodes in ONE call. Prefer this over multiple akg_get_node calls when you need more than one node, so a decision stays a single turn. Returns each node's title, body, meta, tags, and edges; a missing node yields found:false.",
            parameters: Type.Object({
...
function readNodeResult(store, type, id) {
    const node = store?.getNode(type, id) ?? null;
    if (!store || !node)
        return { found: false, type, id };
    const ref = { type, id };
    return {
        found: true,
        ...serializeNode(node),
...
function upsertNode(store, params) {
    try {
...
function upsertEdge(store, params) {
    try {
    ...
    return [
        defineTool({
            ...
            async execute(_toolCallId, params) {
                const store = await getStore();
                if (!store)
                    return jsonResult({ applied: false, error: "no store available" });
        ...
        defineTool({
            name: "akg_put_node",
            label: "AKG Put Node",
            description: "Create or replace a single node. Prefer akg_apply to batch all writes; use this only for a one-off. You choose the type, id, title, body, meta, and tags. Reuse an existing (type, id) to update it in place. Does not delete nodes.",
        ...
        defineTool({
            name: "akg_put_edge",
            label: "AKG Put Edge",
            description: "Create or replace a single directed edge between two existing nodes. Prefer akg_apply to batch all writes; use this only for a one-off. Both endpoint nodes must already exist. Does not delete edges.",
