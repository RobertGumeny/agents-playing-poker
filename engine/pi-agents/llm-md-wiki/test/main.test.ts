...
import { listPages, readPage, ROOT_PAGE, wikiDir } from "../src/pages.js";
...
describe("llm-md-wiki package wiring", () => {
  it("publishes a stable poker-server entrypoint", async () => {
    ...
    expect(pkg.bin).toEqual({ "poker-agent-llm-md-wiki": "./dist/main.js" });
  });

  it("runs as a subprocess, updates the wiki after a hand, and injects the root page into the next decision", async () => {
    const { spawn } = await import("node:child_process");
    const sessionDir = await mkdtemp(path.join(tmpdir(), "llm-md-wiki-session-"));
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once("exit", (code) => {
        if (code !== 0) {
          reject(new Error(`llm-md-wiki exited with code ${code}: ${stderr.read()?.toString("utf8") ?? ""}`));
          return;
        }
        resolve();
      });
    ...
    expect(decoded[0]).toMatchObject({ type: "session_ready", in_reply_to: "msg-1", payload: { version: "llm-md-wiki/0.1.0" } });
