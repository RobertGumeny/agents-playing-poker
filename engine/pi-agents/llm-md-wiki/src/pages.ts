...
export function wikiDir(memoryDir: string): string {
...
export function resolvePagePath(dir: string, slug: string): string | null {
...
export function pageSlug(dir: string, fullPath: string): string {
...
export async function readPage(dir: string, slug: string): Promise<PageReadResult> {
  const full = resolvePagePath(dir, slug);
  if (!full) return { found: false, page: slug };
  try {
...
export async function writePage(dir: string, slug: string, content: string): Promise<string | null> {
  const full = resolvePagePath(dir, slug);
  if (!full) return null;
...
export async function ensureRootPage(dir: string): Promise<void> {
