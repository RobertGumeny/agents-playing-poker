...
export function createReadTools(getWikiDir) {
    return [
        defineTool({
            name: "md_list_pages",
            label: "Wiki Pages",
        ...
        defineTool({
            ...
            async execute(_toolCallId, params) {
                const dir = getWikiDir();
                if (!dir)
                    return jsonResult({ found: false, page: params.page });
                return jsonResult(await readPage(dir, params.page));
            },
        }),
    ];
}
export function createWriteTool(getWikiDir) {
    return defineTool({
        name: "md_write_page",
        label: "Wiki Write",
        ...
        async execute(_toolCallId, params) {
            const dir = getWikiDir();
            if (!dir)
                return jsonResult({ written: false, page: params.page, error: "no wiki directory available" });
