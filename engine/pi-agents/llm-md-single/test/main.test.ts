...
describe("llm-md-single package wiring", () => {
  it("publishes a stable poker-server entrypoint", async () => {
    ...
    expect(pkg.bin).toEqual({ "poker-agent-llm-md-single": "./dist/main.js" });
  });

  it("runs as a subprocess, rewrites notes.md after a hand, and injects them into the next decision", async () => {
    const { spawn } = await import("node:child_process");
    const sessionDir = await mkdtemp(path.join(tmpdir(), "llm-md-single-session-"));
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once("exit", (code) => {
        if (code !== 0) {
          reject(new Error(`llm-md-single exited with code ${code}: ${stderr.read()?.toString("utf8") ?? ""}`));
          return;
        }
        resolve();
      });
    ...
    expect(decoded[0]).toMatchObject({ type: "session_ready", in_reply_to: "msg-1", payload: { version: "llm-md-single/0.1.0" } });
    ...
    // notes.md was written by the post-hand update step.
    const notes = await readFile(path.join(sessionDir, "notes.md"), "utf8");
