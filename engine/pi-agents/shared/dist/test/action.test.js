...
describe("parseActionResponse", () => {
    ...
    it("recovers the action when the model leaks reasoning prose before the JSON", () => {
        ...
        expect(parseActionResponse('QJsuited is a strong hand here.\n\n{"action":"raise","amount":8}')).toEqual({ action: "raise", amount: 8 });
    });
    ...
    it("prefers the final action object when several appear", () => {
        expect(parseActionResponse('I considered {"action":"fold"} but instead {"action":"check"}')).toEqual({ action: "check" });
    });
...
describe("validateOrFallback", () => {
    ...
    it("accepts inclusive raise bounds and rejects out-of-range raises", () => {
        ...
        expect(validateOrFallback({ action: "raise", amount: 21 }, [
            { action: "fold" },
            { action: "raise", min: 10, max: 20 },
        ])).toEqual({ action: "fold" });
    });
    it("falls back to check before fold or call", () => {
        expect(validateOrFallback({ action: "raise", amount: 100 }, [
            { action: "check" },
            { action: "fold" },
            { action: "call", amount: 2 },
