...
describe("PiDecisionEngine", () => {
    it("creates a fresh Pi session per decision and appends to the canonical pi-session artifact", async () => {
        ...
        await expect(engine.decide({ context: { handNumber: 1 }, prompt: "first", legalActions: [{ action: "call", amount: 2 }] })).resolves.toEqual({ action: "call", amount: 2 });
    ...
    it("reuses one Pi session across multiple decisions in a hand and resets it at hand end", async () => {
        ...
        await expect(engine.decide({ context: { handNumber: 7 }, prompt: "first", legalActions: [{ action: "check" }] })).resolves.toEqual({ action: "check" });
        await expect(engine.decide({ context: { handNumber: 7 }, prompt: "second", legalActions: [{ action: "check" }] })).resolves.toEqual({ action: "check" });
        expect(sessionFactory).toHaveBeenCalledTimes(1);
        await engine.onHandEnd?.({ handNumber: 7 });
        await expect(engine.decide({ context: { handNumber: 8 }, prompt: "third", legalActions: [{ action: "check" }] })).resolves.toEqual({ action: "check" });
        expect(sessionFactory).toHaveBeenCalledTimes(2);
    ...
    it("falls back to streamed assistant text when needed and surfaces malformed JSON", async () => {
        ...
        await expect(engine.decide({ context: { handNumber: 1 }, prompt: "decision", legalActions: [{ action: "fold" }] })).rejects.toThrow('pi decision failed: assistant returned malformed action JSON: "{\\"action\\":\\"dance\\"}"');
    });
});
