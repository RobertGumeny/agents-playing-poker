...
describe("llm-stateless package wiring", () => {
  it("publishes a stable poker-server entrypoint", async () => {
    ...
    expect(pkg.bin).toEqual({
      "poker-agent-llm-stateless": "./dist/main.js",
    });
  });
  ...
  it("runs as a subprocess with repeated extra args, replies over stdio, and writes the canonical pi-session artifact path", async () => {
    ...
    child.stdin.write(JSON.stringify({ v: 1, type: "session_end", id: "msg-4", payload: {} satisfies Record<string, never> }) + "\n");
    child.stdin.end();
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once("exit", (code) => {
        if (code !== 0) {
          reject(new Error(`llm-stateless exited with code ${code}: ${stderr.read()?.toString("utf8") ?? ""}`));
          return;
        }
        resolve();
      });
